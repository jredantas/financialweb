...
py.sign_in(
    username=config_data["plotly"]["username"], api_key=config_data["plotly"]["api_key"]
)
...
def import_expense():
    ...
    try:
        df = pd.read_csv(
            "/home/renato/Documents/financialweb/data/despesas.csv", sep=";"
        )
...
def accueil():
    ...
    if "username" in session:
        return render_template(
            "accueil.html", titre="Financial web", username=session["username"]
        )
...
def profile():
    if len(session) == 0:
        return render_template(
            "accueil.html", titre="Financial Web", alert="User not logged in."
        )
...
def login():
    ...
    if request.method == "POST":  # and username == 'admin':
        try:
            ...
            result = (
                dbsession.query(table).filter(table.columns.email == username).first()
            )
            ...
            return render_template(
                "accueil.html",
                titre="Financial Web",
                alert="It was not possible to retrieve the information. Please try again.",
            )

    return render_template(
        "accueil.html", titre="Financial Web", alert="Username or password not correct."
    )
...
def insert(instance):
    ...
    try:
        ...
        labels, columns = zip(
            *filter(
                lambda x: primaryKeyColName.title() not in x, zip(labels, table.columns)
            )
        )
...
def update(instance, id):
    ...
    return render_template(
        "update.html", titre="Financial web - " + instance, section_titre=instance
    )
    ...
    if request.method == "POST":
        if request.form["inputPassword1"] != request.form["inputPassword2"]:
            return render_template(
                "signup.html",
                titre="Financial Web",
                alert="Password fields must be equal. Try again.",
            )
    ...
    if request.method == "POST":
        try:
            ...
            contact = Contact(
                request.form["inputName"],
                request.form["inputEmail"],
                request.form["formControlMessage"],
            )
            ...
            return render_template(
                "accueil.html",
                titre="Financial Web",
                alert="Contact registered. Thank you.",
            )
        except Exception as err:
            print(err)
            return render_template(
                "accueil.html",
                titre="Financial Web",
                alert="Contact not registered. Try again.",
            )
    ...
    if request.method == "POST":
        try:
                    ...
                    if request.form[column.name] != "":
                        ...
                        if isinstance(column.type, mysql.DECIMAL):
                            values[column.name] = float(
                                request.form[column.name]
                            )  # datetime.datetime.strftime(datetime.date(request.form[column.name]), '%Y-%m-%d')
                        if isinstance(column.type, mysql.INTEGER):
                            values[column.name] = int(
                                request.form[column.name]
                            )  # datetime.datetime.strftime(datetime.date(request.form[column.name]), '%Y-%m-%d')
            ...
            return render_template(
                "accueil.html",
                titre="Financial Web",
                alert="It was not possible to insert the record. Try again.",
            )
...
def remove_save(instance):
    if request.method == "POST":
        return render_template(
            "accueil.html",
            titre="Financial Web",
            alert="Entrou no metodo. " + request.form["id"],
        )
...
def month_expense():
    ...
    try:
        ...
        result = (
            ...
            .filter(table.columns.due_date <= filter_to)
            .all()
        )
    except Exception as err:
        print(err)
...
def family_expense():
    ...
    try:
        ...
        resultset = (
            ...
            .join(Account)
            .filter(Expense.due_date >= filter_from)
            .filter(Expense.due_date <= filter_to)
            .order_by(Expense.account_id)
            .all()
        )
        result = []
        ...
        for res in resultset:
            ...
            result.append(
                (
                    ...
                    res[0].due_date,
                    res[0].amount,
                    str(res[0].installment) + "/" + str(res[0].installment_group),
                    res[1].description,
                )
            )
    ...
    return render_template(
        "family_expense.html",
        titre="Financial web - Family Expense",
        elements=result,
        columns=labels,
        total_amount=total_amount,
        dropdownvalue=request.args.get("filters"),
    )
...
def family_expense_chart():
    ...
    try:
        ...
        resultset = (
            dbsession.query(
                func.DATE_FORMAT(Expense.due_date, "%Y-%m"), func.sum(Expense.amount)
            )
            .group_by(func.DATE_FORMAT(Expense.due_date, "%Y-%m"))
            .all()
        )
        for res in resultset:
            labels.append(res[0])
...
def expense_income_chart():
    ...
    try:
        ...
        rsIncome = (
            dbsession.query(
                func.DATE_FORMAT(Income.pay_date, "%Y-%m"), func.sum(Income.amount)
            )
            .group_by(func.DATE_FORMAT(Income.pay_date, "%Y-%m"))
            .all()
        )
        for rsi in rsIncome:
            # labels.append(rsi[0])
...
def mpl_chart():
    ...
    try:
        ...
        chart_url = py.plot_mpl(
            chart, filename="expense_income_monthly", auto_open=False
        )
    ...
    return render_template(
        "mpl_chart.html", titre="Financial web - Family Expense", chart_url=chart_url
    )
